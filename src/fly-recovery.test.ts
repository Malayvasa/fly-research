...
test('low assisted throttle recovers a sustained stall after one and a half seconds', () => {
  const recovery = new FlyRecovery();
  for (let i = 0; i < 14; i++) assert.equal(recovery.observe(0.1, 0.1, 0.3, true), false);
  let triggered = false;
...
test('motion, inactivity, no throttle and invalid observations cancel accumulated stall', () => {
  for (const interrupt of [[0.1,1,0.3,true],[0.1,0,0.3,false],[0.1,0,0,true],[NaN,0,0.3,true]] as const) {
    const recovery = new FlyRecovery();
    for(let i=0;i<10;i++) recovery.observe(0.1,0,0.3,true);
    assert.equal(recovery.observe(...interrupt),false);
    for(let i=0;i<10;i++) assert.equal(recovery.observe(0.1,0,0.3,true),false);
